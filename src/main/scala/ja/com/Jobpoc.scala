...
//    s3://estestoutput1/sourceloc/BL-100267.cdx
//    s3://estestoutput1/sourceloc/BL-100267.arc.gz
// s3://estestoutput1/sourceloc/SearchWhitelist/dns_*

    val format = new java.text.SimpleDateFormat("yyyyMMddHHmmss")
    var outputrootFolderFormat = "s3://estestoutput1/targetloc/%s"

    var arcfile = "s3://estestoutput1/sourceloc/BL-100267.arc.gz" //give  thek full ofile  name
    var cdxfile = "s3://estestoutput1/sourceloc/BL-100267.cdx"

   // var dnsfiles = "/whiltelist/dns_*" //ToDO: restore the correct settings linked to AWS
    var dnsfiles = "s3://estestoutput1/sourceloc/SearchWhitelist/dns_*"
...
dnsfiles = "C:\\Users\\Ja\\Google Drive\\oldfiles\\Note\\srcfile\\SearchWhitelist\\dns_*"
   ...
   // System.setProperty("HADOOP_USER_NAME", username)
    ...
    filteredCDXitems.select("m_mime_type_of_original_document").distinct().rdd.coalesce(1)
      .saveAsTextFile(outputrootFolderFormat.format("distinct_mimetype_output_"+ format.format(new java.util.Date() )))

    println(" 12 ############## $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ ")

    // fDf1.coalesce(1).write.save(outputrootFolderFormat.format("txt_finaloutput_"  + format.format(new java.util.Date())))

    filteredCDXitems.show(10)

    filteredCDXitems.createOrReplaceTempView("mainT")
    //filteredCDXitems.registerTempTable("mainT")

    val domainsDf =  getDomainsDf(dnsfiles)

    domainsDf.createOrReplaceTempView("domainT")
...
//Todo : for hdfs file
 /*   val file = new File(arcfile)
    ...
    val fstream : InputStream = readHdfsFile(arcfile) // if(!filelocation.equals("local"))  else None
   ...
   // val file = new File(arcfile)                                 //  val file1 = new File(arcfile)   thinking about hadoop files..this will work for local .. not for hadoop
    ...
    val lstFinalDataToSave = joinedDF.collect().map(cdxItem => {
      ...
      val arcReader = ARCReaderFactory.get("arc", fstream, true )    //.get(arcfile)
...
//1 sec sir .. complete the final step
   /* line: Int, arcTime:String, mimeType:String, url:String,
    ...
    fDf_demo1.rdd.coalesce(1).saveAsTextFile(outputrootFolderFormat.format("pdf_pdf_output_"  + format.format(new java.util.Date())))
    ...
    fDf_demo2.rdd.coalesce(1).saveAsTextFile(outputrootFolderFormat.format("doc_doc_output_"  + format.format(new java.util.Date())))
    ...
    fDf_demo3.rdd.coalesce(1).saveAsTextFile(outputrootFolderFormat.format("html_html_output_"  + format.format(new java.util.Date())))
    ...
    fDf_demo4.rdd.coalesce(1).saveAsTextFile(outputrootFolderFormat.format("vnd_ms-excel_vnd_ms-excel_"  + format.format(new java.util.Date())))
    ...
    fDf_demo5.rdd.coalesce(1).saveAsTextFile(outputrootFolderFormat.format("plain_plain_output_"  + format.format(new java.util.Date())))
    ...
    fDf_demo6.rdd.coalesce(1).saveAsTextFile(outputrootFolderFormat.format("vnd_ms-powerpoint_vnd.ms-powerpoint_output_"  + format.format(new java.util.Date())))
    ...
    fDf_demo7.rdd.coalesce(1).saveAsTextFile(outputrootFolderFormat.format("octet-stream_octet-stream_output_"  + format.format(new java.util.Date())))
    ...
    */
    ...
    //fDf1.write.format("avro").save(outputrootFolderFormat.format("finaloutput_"  + format.format(new java.util.Date())))

    fDf1.coalesce(1).write.save(outputrootFolderFormat.format("txt_finaloutput_"  + format.format(new java.util.Date())))

    println(" 42 ############## $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ ")

    fDf1.coalesce(1).write.json(outputrootFolderFormat.format("json_finaloutput_"  + format.format(new java.util.Date())))
